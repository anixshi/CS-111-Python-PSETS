...
def displayPic (pic):
    ...
    framedPic.scale(3)
    frame.add(framedPic)
    return frame # So canvas can be referenced elsewhere, as in savePicToFile

# Lyn sez: This will only work if PIL is imported, and cs1graphics needs to 
# be tweaked do to that
def savePicToFile(pic, filename):
    frame = displayPic(pic)
