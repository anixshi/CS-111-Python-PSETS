""" This is cs1graphicsAngle.py, a version of cs1graphics.py
    edited by Lyn to provide a getAngle() method for drawable objects
    that approximates how much they have been rotated. """
...
class Drawable(_EventTrigger):
    ...
    def getAngle(self):
        """[2016-12-03, added by Lyn]"""
        # Following code copied from rotate()
        p = self._localToGlobal(self._reference)
        trans = _Transformation((1.,0.,0.,1.)+p.get())
        matrix = (trans.inv()*self._transform)._matrix
        # We test rotation of unit vector (1,0)
        # x of result is matrix[0], y of result is matrix[2]
        return _math.atan2(matrix[2],matrix[0])*180/_math.pi
